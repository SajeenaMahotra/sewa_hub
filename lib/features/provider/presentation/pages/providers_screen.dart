...
class ProvidersScreen extends ConsumerStatefulWidget {
  ...
  final bool isEmergency; // ← NEW

  const ProvidersScreen({
    super.key,
    this.initialSearch,
    this.initialCategory,
    this.isEmergency = false, // ← default false
  });
...
class _ProvidersScreenState extends ConsumerState<ProvidersScreen> {
  ...
  bool _loadingEmergency = false; // overlay while GPS fetches
  ...
  // ── Emergency tap: GPS → reverse geocode → CreateBookingPage ─────────────
  Future<void> _onEmergencyProviderTap(ProviderEntity provider) async {
    setState(() => _loadingEmergency = true);
    ...
    try {
      ...
      // silently fall back to empty address
    }

    if (!mounted) return;
    setState(() => _loadingEmergency = false);
  ...
  List<ProviderEntity> _applyFiltersAndSort(List<ProviderEntity> providers) {
    var result = providers.where((p) {
      ...
      final matchesPrice  = p.pricePerHour >= _priceRange.start &&
          p.pricePerHour <= _priceRange.end;
      final matchesExp    = _minExperience == 0 ||
          p.experienceYears >= _minExperience;
  ...
  void _showFilterSheet(BuildContext context, List<ProviderEntity> providers) {
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _FilterSheet(
        ...
        onApply: (cat, sort, range, exp) => setState(() {
  ...
  String _sortLabel(_SortOption o) {
    switch (o) {
      ...
      case _SortOption.expHigh:    return 'Most Experienced';
      case _SortOption.none:       return '';
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final cats     = _getCategories(state.providers);
                      ...
                      children: [
                        Row(
                          children: [
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  if (widget.isEmergency)
                                    const Text(
                                      'Tap a provider to book instantly',
                                      style: TextStyle(
                            ...
                            // Hide filter button in emergency mode
                            if (!widget.isEmergency)
                              GestureDetector(
                        ...
                        children: [
                          ...
                          if (_sortOption != _SortOption.none) ...[
                            ...
                            Container(
                              ...
                              child: Text(_sortLabel(_sortOption),
                                  style: const TextStyle(
                                      fontSize:   10,
                                      fontWeight: FontWeight.w600,
                                      color:      _orange)),
    ...
    return RefreshIndicator(
      ...
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          ...
          return ProviderCard(
            provider: provider,
            // ← Emergency tap vs normal tap
            onTap: widget.isEmergency
                ? () => _onEmergencyProviderTap(provider)
                : () => Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) =>
...
class _FilterSheet extends StatefulWidget {
  ...
  final void Function(String, _SortOption, RangeValues, int) onApply;
          ...
          children: [
            ...
            _SheetSection(
              ...
              child: Wrap(
                ...
                children: [
                  _SortChip(label: 'Default',           value: _SortOption.none,       selected: _sort, onTap: (v) => setState(() => _sort = v)),
                  _SortChip(label: 'Price: Low → High', value: _SortOption.priceLow,   selected: _sort, onTap: (v) => setState(() => _sort = v)),
                  _SortChip(label: 'Price: High → Low', value: _SortOption.priceHigh,  selected: _sort, onTap: (v) => setState(() => _sort = v)),
                  _SortChip(label: 'Top Rated',         value: _SortOption.ratingHigh, selected: _sort, onTap: (v) => setState(() => _sort = v)),
                  _SortChip(label: 'Most Experienced',  value: _SortOption.expHigh,    selected: _sort, onTap: (v) => setState(() => _sort = v)),
                ],
              ),
            ),
            _SheetSection(
              ...
              child: Column(
                children: [
                  Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      _PriceLabel('NPR ${_priceRange.start.toStringAsFixed(0)}'),
